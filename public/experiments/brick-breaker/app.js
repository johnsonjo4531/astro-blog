(function (ViewSwitcher, MenuView, GameView) {
  ...
  function main (e) {
    ...
    const CreditsView = ViewStarters.ButtonView('CreditsView', $('#credit-display')[0], () => {
      MainView.loadView(MenuView.name);
    });
    const HighScoresView = ViewStarters.ButtonView('HighScoresView', $('#high-scores-display')[0], () => {
      MainView.loadView(MenuView.name);
    });
    const ControlsView = ViewStarters.ButtonView('ControlsView', $('#controls-menu')[0], () => {
      MainView.loadView(MenuView.name);
    });
    window.MainView = ViewSwitcher();
    MainView.addViews(MenuView, GameView, CreditsView, HighScoresView, ControlsView);
    MainView.loadView(MenuView.name);
